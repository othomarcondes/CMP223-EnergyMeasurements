...
ggsave("figures/stress_energ.pdf", width=6, height=5)
#+end_src
...
p <- energ.used |>
  ...
  geom_point(size = 2) +
  facet_grid(type_24 ~ period,
             scales = "fixed",
             labeller = labeller(
      period = c(
        "< 2025-11-26"  = "Before fan repair",
        ">= 2025-11-26" = "After fan repair"
      ))
      ) +
  default_theme(base_size = 12) +
  ylim(0, NA) +
  labs(
    x = "Nodes",
    y = "Energy (Wh)",
...
ggsave("figures/stress_energ_facet.pdf", width=6, height=5)
#+end_src
...
ggsave("figures/stress.pdf", width = 6, height = 5)
#+end_src
...
ggsave("figures/lu_times.pdf", width=6, height=5)
#+end_src
...
ggsave("figures/lu_energ.pdf", width=6, height=5)
#+end_src
...
ggsave("figures/lu.pdf", width = 6, height = 5)
#+end_src
...
lu.result |>
  ...
  ungroup() |>
  ggplot(aes(x = t_comp, y = Active_Power, color = factor(idx))) +
  default_theme(base_size = 10) +
  ylim(0, NA) +
  geom_line() +
  facet_grid(~ nodes, scales = "fixed") +
  labs(x = "Time (s)", y = "Active Power (W)") -> p

ggsave("figures/lu_facet.pdf", width = 6, height = 5)
#+end_src

#+RESULTS:

**** Facet view no outlier
#+begin_src R :results output :session *R* :exports both :noweb yes :colnames yes
lu.result |>
  filter(order != "lu_factor_poti_60000_3_GPU0_9") |>
  arrange(idx, Time) |>
  group_by(idx) |>
  mutate(
    dt = as.numeric(difftime(Time, lag(Time), units = "secs")),
...
ggsave("figures/lu_facet_no_out.pdf", width = 6, height = 5)
#+end_src
